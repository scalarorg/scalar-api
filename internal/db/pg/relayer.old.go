...
func (c *PostgresClient) GetRelayerData(ctx context.Context, options *models.Options) ([]relayer.RelayData, int, *types.Error) {
	...
	log.Ctx(ctx).Debug().Msg(fmt.Sprintf("GetRelayerData with Event Id: %s", options.EventId))
	if options.EventId != "" {
	...
	defer rows.Close()
	for rows.Next() {
		relayData := relayer.RelayData{
			// CallContract: relayer.CallContract{
			// 	ContractCallApproved: relayer.ContractCallApproved{},
			// },
...
// func (c *PostgresClient)GetRelayerData0(ctx context.Context, options *Options) ([]RelayData, *types.Error) {
...
func (c *PostgresClient) GetContractCallParams(ctx context.Context, messageIds []string) (map[string]relayer.CallContract, *types.Error) {
	mapContractCalls := make(map[string]relayer.CallContract)
	var contractCalls []relayer.CallContract
	result := c.DB.Where("id IN ?", messageIds).Find(&contractCalls)
	if result.Error != nil {
		return nil, types.NewError(http.StatusInternalServerError, types.InternalServiceError, result.Error)
	}
	for _, contractCall := range contractCalls {
		mapContractCalls[contractCall.EventID] = contractCall
	}
...
// func (c *PostgresClient) GetExecutedVaultBonding(ctx context.Context, options *models.Options) ([]relayer.RelayData, *types.Error) {
// 	var relayDatas []relayer.RelayData
// 	if options.Size <= 0 {
// 		options.Size = 10
// 	}
...
// 			fmt.Printf("Error while scanning rows %v", err)
// 		}
// 		relayData.CallContract.EventID = relayData.ID
// 		//relayData.ContractCallWithToken.ID = relayData.ID
// 		relayDatas = append(relayDatas, relayData)
// 		// do something
// 	}
